...
export default function CDramaPage() {
  ...
  useEffect(() => {
    setLoading(true);
    let url = `${BASE_URL}/discover/tv?api_key=${API_KEY}&with_origin_country=CN&sort_by=${sort}&page=${page}&vote_count.gte=10&with_original_language=zh&without_genres=16,10762`;
    ...
    <main className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <div className="px-4 sm:px-8 py-6">
              ...
              {genres.map(g => (
                <button key={g} onClick={() => { setGenre(g); setPage(1); }}
                  className={`px-3 py-1 rounded-full text-xs font-medium transition
                    ${genre === g ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  {g}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {["All", "Ongoing", "Completed", "Upcoming"].map(s => (
                <button key={s} onClick={() => { setStatus(s); setPage(1); }}
                  className={`px-3 py-1 rounded-full text-xs font-medium transition
                    ${status === s ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  {s}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <button key={s.value} onClick={() => { setSort(s.value); setPage(1); }}
                  className={`px-3 py-1 rounded-full text-xs font-medium transition
                    ${sort === s.value ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                  {s.label}
                </button>
              ))}
            </div>
        ...
        <div className="flex justify-center gap-4 mt-10">
          ...
          <span className="px-6 py-2 bg-amber-500 text-white rounded-lg text-sm">Page {page}</span>
